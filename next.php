...
if ($jam >= 20) {
    $tanggal = date("D"); // Mendapatkan tanggal saat ini

    $tanggal_mendatang = date("D", strtotime($tanggal . " +1 day")); // Menambahkan 1 hari ke tanggal saat ini

    $hari = date("D", strtotime($tanggal_mendatang));
...
Menginformasikan piket Instalasi SIMRS (ext 2000)<?php echo " Hari ini : ". hari_ini(); ?>, <?php  $currentDate = date("d"); // Mendapatkan tanggal saat ini (format: YYYY-MM-DD)
$currentHour = date("H");

if ($currentHour >= 20) { // Jika sudah pukul 8 malam
    $newDate = date("d", strtotime("+1 day")); // Tambahkan 1 hari
} else {
    $newDate = $currentDate; // Tidak perlu menambahkan 1 hari
}

echo $newDate .' '.($bulan[date('m')]).' '.date('Y') ?> Jam <?php 
$t = date("H");
$t = strtotime("+7 hours"); 
 if ($t >= 24) { 
        $t = 20;
...
if ($t == 20) {
    // Reset waktu menjadi jam 00 (midnight)
    $t = 0;
}
if ( $t < "4") {
    echo "07.00 sd 14.00";
...
<?php $nomor=1; ?>
		    ...
		    $currentHour = date("H");
			if ($currentHour >= 20) {
			$tglnow = idate("d", strtotime("+1 day")); }
			?>
